...
has data => (isa => 'HashRef', is => 'ro' );

with qw(Bender::Role::Storage::YAML);

has irc => ( isa => 'Obj', is => 'ro' );

has session_id => ( isa => 'Str', is => 'ro' );
...
has owner => (
    isa     => 'Str',
    is      => 'ro',
    default => sub { 'perigrin!~perigrin@c-75-72-134-35.hsd1.mn.comcast.net' },
);


sub BUILD {
    my ( $self, $param ) = @_;
    $self->{data_file} = $param->{Trustfile} || $self->Trustfile;
    POE::Session->create(
        object_states => [
            $self => [
                qw(
	...
	$self->meta->add_method(S_msg => \&S_bot_addressed);
}
...
sub S_mode {
    ...
    while ( my $mode = shift( @{ $parsed_mode->{modes} } ) ) {
      ...
      SWITCH: {
            ...
            {
                $poe_kernel->post( $self->{session_id}, '_spread_ops',
                    $channel );
                last SWITCH;
            }
...
sub check {
    ...
    return 1 if $user eq 'apeiron' && $host eq 'shadow.cat';
    return 1 if $user eq 'matthewt' && $host eq 'shadow.cat';
...
my $spread_love = qq[^(?:(?:spread (?:the love|ops))|(?:names))\\s*(?:in\\s+$CHANNEL)?[?.!]*];
